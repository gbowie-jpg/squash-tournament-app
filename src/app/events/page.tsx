        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
              ...
              {allTournaments.map((t) => {
                ...
                return (
                  <Link
                    key={t.id}
                    href={`/t/${t.slug}`}
                    className="block bg-card rounded-xl border border-border p-5 hover:border-blue-300 hover:shadow-md transition-all"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="font-semibold text-lg">{t.name}</h3>
                        {t.venue && <p className="text-muted-foreground text-sm mt-1">{t.venue}</p>}
                        <p className="text-muted-foreground text-sm mt-1">{dateStr}</p>
                        {t.description && <p className="text-muted-foreground text-sm mt-2">{t.description}</p>}
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              <p className="text-muted-foreground text-sm">Annual tournament held every October. Open to all skill levels with multiple draws.</p>
            </div>
            <div className="bg-card rounded-xl border border-border p-6">
              <h3 className="font-semibold text-lg mb-2">Seattle City Championships</h3>
              <p className="text-muted-foreground text-sm">Held each April/May. Crowns city champions across divisions.</p>
            </div>
            <div className="bg-card rounded-xl border border-border p-6">
              <h3 className="font-semibold text-lg mb-2">Referee Clinic</h3>
              <p className="text-muted-foreground text-sm">Training sessions for aspiring referees. Learn the rules and officiating best practices.</p>
            </div>
            <div className="bg-card rounded-xl border border-border p-6">
              <h3 className="font-semibold text-lg mb-2">Junior Scholar Athlete Award</h3>
              <p className="text-muted-foreground text-sm">Recognizing outstanding young players who excel both on court and in academics.</p>
            </div>
          </div>
        </div>
      </main>
