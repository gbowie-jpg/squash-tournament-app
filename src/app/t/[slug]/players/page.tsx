      ...
      <main className="max-w-3xl mx-auto px-4 py-4">
        {loading ? (
          ...
          <div className="text-center py-16">
            <Search className="w-8 h-8 text-dim mx-auto mb-3" strokeWidth={1.5} />
                ...
                {filtered
                  ...
                  .map((p, i) => (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-4 py-3.5 hover:bg-surface active:bg-surface transition-colors ${
                        i > 0 ? 'border-t border-[var(--border)]' : ''
                      }`}
                    >
                      {p.seed ? (
                        <span className="w-7 h-7 rounded-full bg-surface text-muted-foreground text-xs font-bold flex items-center justify-center flex-shrink-0">
                          {p.seed}
                        </span>
                      ) : (
                        <span className="w-7 h-7 rounded-full bg-surface flex-shrink-0" />
                      )}
