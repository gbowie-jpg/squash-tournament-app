  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b border-border sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            ...
            <span>›</span>
            <Link href={`/t/${slug}/admin`} className="hover:text-foreground">{tournament?.name ?? slug}</Link>
          </div>
          <div className="flex items-center gap-2 mt-1">
            <Link href={`/t/${slug}/admin`} aria-label="Back to tournament admin" className="text-muted-foreground hover:text-foreground p-1 -ml-1">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <form onSubmit={handlePublish} className="bg-card border border-border rounded-xl p-6 mb-8 space-y-4">
          <textarea
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            placeholder="Type your announcement..."
            rows={3}
            className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-surface text-foreground placeholder:text-dim focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-zinc-400"
          />
            ...
            {announcements.map((a) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button onClick={() => handleDelete(a.id)} aria-label="Delete announcement" className="text-xs text-red-400 hover:text-red-500 shrink-0">
                    Delete
                  </button>
                </div>
              </div>
